...
file_name = r'F:\WORK\AdFox\Справочники\Для Admon и Brain\puids_new_2022_{}.csv'\
...
with writer:
    while rows + (page - 1) * limit < total_rows:
        ...
        for row in data:
            site_data = {}
            site_data["puid1"] = row.find("siteID").text
            site_data["puid1_name"] = row.find("siteName").text
            site_data["puid2"] = row.find("ID").text
            site_data["puid2_name"] = row.find("name").text.replace(",", ";")
